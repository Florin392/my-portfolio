...
import { coursesData, workData, educationData } from "../../data.js";
...
export default function Experience() {
  ...
  return (
    <div
      ...
      id="experience"
      style={{
        backgroundColor: setdarkMode || "whiteSmoke",
        backgroundImage:
          setdarkMode ||
        ...
        {data.map((d, index) => (
          <Tilt className="item" options={options} key={index}>
            <div className="itemDetails">
              ...
              <h4>{d.date}</h4>
              <h6>
                {d.description.map((el, indexel) => (
                  <li key={indexel}>{el}</li>
                ))}
              </h6>
              <h5>{d.company}</h5>
            </div>
            ...
            <div className="hoverContainer">
              ...
              <h2 style={{ color: setdarkMode && "yellow" }}>
                {d.hoverEffectTitle.map((el, indexEl) => (
                  <p key={indexEl}>{el}</p>
                ))}
              </h2>
              <h3 style={{ color: setdarkMode && "yellow" }}>
                {d.hoverEffectDescription.map((el, indexelL) => (
                  <p key={indexelL}>{el}</p>
                ))}
              </h3>
            </div>
